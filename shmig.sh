...
# Downloaded from https://github.com/mbucc/shmig/blob/master/shmig
...
if [ "${ENVIRONMENT}" == "development" ]; then
  ...
  CONFIG="./shmig-production.conf"
else
  echo "Unknown enviroment '${ENVIRONMENT}'. Please specify either 'development', 'staging', or 'production' and try again."
  exit 1
fi
...
die(){
  cat >&2 <<EOF
Usage: $ME [options] <create|up|down|rollback|status|redo|pending> [arguments...]
Common options:
  -d <database name>
...
[[ $ASK_PASSWORD -eq 1 ]] && read -s -p "Password for $LOGIN: " _PASSWORD && echo

[ "development" != "$ENVIRONMENT" ] && \
  [[ -z "${_PASSWORD}" ]] && \
  [[ -z "${PASSWORD}" ]] && \
  echo && \
  echo "${RED}PASSWORD not provided for env '$ENVIRONMENT' and is required${CLEAR}. " && \
  echo "Consider using the -A flag to prompt for password and prevent the password from showing in bash history" && \
  echo && \
  exit 1
